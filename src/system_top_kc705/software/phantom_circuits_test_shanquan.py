...
def analysis(file,num_measurements,tx,tx_bits_ref,num_tx_bits,cooldown_cycles,delay_cycles):
    ...
    for i in range(4):
        # print(data[i], "\n")
        for j in range(len(tx_bits_diff[i])):
            if(tx_bits_diff[i][j] < 0):
                tx_bits[i].append(1)
            else:
                tx_bits[i].append(0)
    ...
    #         intersect = list(set(lst1) & set(lst2))
            # index_intersect[k].append(list(set(lst1) & set(lst2)))
    ...
    print("")
    for i in range(4):
        print(expected_offset[i])

    #######################################
    # Plot Best offset that gives you the correct bit
    if(tx == None):
        txbit = "random"
    else:
        txbit = str(tx)
    ...
    for i in range(num_measurements):
        for j in range(4):
            for k in range(len(expected_offset[j][i])):
                if(expected_offset[j][i][k] == k):
                    plt.plot(x[k],j, "ro")

                # if(expected_offset[j][i][k] == 1):
                #     plt.plot(x[k],j, "go")
                
                # if(expected_offset[j][i][k] == 2):
                #     plt.plot(x[k],j, "bo")

                # if(expected_offset[j][i][k] == 3):
                #     plt.plot(x[k],j, "ko")
        x += tx_array  
    ...
    plt.show()
    plt.savefig("best_offset_random_" + str(num_tx_bits) + "_bits"+ ".png")
                ...
                # plt.ylim(-1,100)
                # plt.show()
                # plt.savefig("counter_" + str(i) +"_txbit_random_" + str(int(num_tx_bits)) + "_bits_measure" + str(num_measurements) + "_cooldown" + str(cooldown_cycles) + "_cycles_per_bit" + str(delay_cycles) + ".png")
                # plt.clf()
    #######################################
