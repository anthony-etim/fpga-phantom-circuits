...
// RO HEATER STATES
localparam RO_HEATER_CYCLES         = 5'd14;
...
// start the ro heater
(* mark_debug = "true" *) wire        start_ro_heater;
        ...
        case (state)
            ...
            GET_DATA_PREDICTED: begin
                if (out32counter == 32'd0) begin
                    ...
                    state <= WAIT_RETURN;
                end else if (out32counter <= 32'd15) begin
                    // values: 001, 010, 011
                    if(!tx_busy) begin
                        tx_valid <= 1'b1;
                        tx_byte <= data_predicted[7:0] ;
...
reg [$clog2(MAX_BIT_SIZE)-1:0] num_tx_bits = 0;
...
wire [$clog2(MAX_BIT_SIZE)-1:0] index [0:NUM_RO_COUNTER-1];
wire [$clog2(MAX_BIT_SIZE)-1:0] index_0;
wire [$clog2(MAX_BIT_SIZE)-1:0] index_1;
wire [$clog2(MAX_BIT_SIZE)-1:0] index_2;
wire [$clog2(MAX_BIT_SIZE)-1:0] index_3;
...
localparam FIFO_WRITE_RO_HEATER_ON_NUM       = 3'd3;
