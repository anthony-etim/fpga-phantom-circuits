...
def parse_file(infile):
    ...
    for row in reader:
        ...
        if transmitter_bit is None:
            transmitter_bit = row['transmitter_bit_pred']
        else:
            if transmitter_bit != row['transmitter_bit_pred']:
...
def get_success_rate(all_stats, exp_params, key='diffs', flip=False):
    ...
    pattern = exp_params['pattern_pred']
...
def parse_all_experiments(basedir, key='diffs', regex='.*', flip=False):
    ...
    for exp in sorted(os.listdir(basedir)):
        ...
        for name in sorted(os.listdir(expdir)):
            ...
            if 'pattern' in exp:
                name = ''.join([str(x) for x in exp_params['pattern_pred']])
            ...
            for k in all_keys:
                ...
                cur_stat = {
                    ...
                    'correct': correct,
                    'total': total,
                    'pattern_pred': ''.join([str(s) for s in pattern]),
                    'key': k,
...
def write_all_stats(stats, outfile):
    fieldnames = ['experiment',  'name', 'pattern_pred', 'key', 'correct', 'total', 'percentage']
    if outfile:
        f = open(outfile, 'w')
    else:
        f = sys.stdout
