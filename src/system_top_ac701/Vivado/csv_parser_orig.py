...
from utilities_ilias import makedirs

def parse_file(infile):
    basic_cols = set(['rep', 'heat_enabled'])
    ro_cols = []
    ...
    for row in reader:
        ...
        if transmitter_bit is None:
            transmitter_bit = row['transmitter_bit_orig']
        else:
            if transmitter_bit != row['transmitter_bit_orig']:
...
def get_success_rate(all_stats, exp_params, key='diffs', flip=False):
    ...
    pattern = exp_params['pattern_orig']
    mask = exp_params['mask']
    pattern_len = exp_params['num_tx_bits']
...
def parse_all_experiments(basedir, key='diffs', regex='.*', flip=False):
    ...
    for exp in sorted(os.listdir(basedir)):
        ...
        for name in sorted(os.listdir(expdir)):
            ...
            if 'pattern' in exp:
                name = ''.join([str(x) for x in exp_params['pattern_orig']])
            ...
            for k in all_keys:
                ...
                cur_stat = {
                    ...
                    'correct': correct,
                    'total': total,
                    'pattern_orig': ''.join([str(s) for s in pattern]),
                    'key': k,
...
def write_all_stats(stats, outfile):
    fieldnames = ['experiment',  'name', 'pattern_orig', 'key', 'correct', 'total', 'percentage']
    if outfile:
        f = open(outfile, 'w')
    else:
        f = sys.stdout
...
if __name__ == "__main__":
    ...
    if args.measdir:
        ...
        expdir = os.path.dirname(os.path.abspath(__file__)) + '/phantom_ro_measurements/'
