...
RECEIVER_KC705_SERIAL       = '210203A7DC87A'   # kc705-01 serial no
# RECEIVER_KC705_SERIAL       = '210203A7D6C2A'   # kc705-02 serial no

RECEIVER_KC705_UART_PORT    = '/dev/ttyUSB0'    # kc705-01 uart port
# RECEIVER_KC705_UART_PORT    = '/dev/ttyUSB3'    # kc705-02 uart port
...
class Communicator(object):
    ...
    def __init__(self, port=RECEIVER_KC705_UART_PORT,  baudrate=115200, parity=serial.PARITY_ODD):
    ...
    def upload_one(bitfile):
        ...
        cmd = [vivado, '-mode', 'batch', '-source', uploadfile,
               '-nojournal', '-nolog', '-tclargs', bitfile, RECEIVER_KC705_SERIAL]
        ret = subprocess.call(cmd)
        if ret != 0:
            raise Exception("System call returned %d. Aborting." % ret)

    def upload(self):
        self.upload_one(get_script_dir() + 'binaries/covert_receiver_top.bit')
