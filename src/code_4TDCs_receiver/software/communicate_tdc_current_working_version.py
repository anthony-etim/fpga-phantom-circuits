...
TRANSMITTER_UART_PORT    = '/dev/ttyUSB5'    # kc705-02 uart port
# TRANSMITTER_UART_PORT    = '/dev/ttyUSB4'    # ac701-01 uart port
...
RECEIVER_UART_PORT    = '/dev/ttyUSB4'    # ac701-01 uart port
...
def vary_fpga():
    ...
    kc705_no_trans = 14
    ac701_no_trans = 10
    # for num_trans in range(kc705_no_trans,-1,-2):
    # kc705_trans_mask = (1 << num_trans) - 1
    # ac701_trans_mask = (1 << num_trans) - 1
    for heater_no in range(8,-1,-2):
        parameters = {
            'measurements': 500,
            # 'mask': 0x1f,
            # 'repetitions': 4,
...
class Communicator(object):
    ...
    def upload(self):
        self.upload_one(get_build_dir() + FPGA_RECV + '/synth_system.bit', self.recv_serial_no)
    ...
    def communicate_pattern(self, params):
        ...
        for i, bit in enumerate(pattern):
            ...
            for j in range(repetitions):
                ...
                # sleep_time = 10 # make as global param later
                # heater_no = 4 # make as global param later
    ...
    def set_up_tdc (self, ser, measures, config = 0xff, coarse = 0xC, fine = 0x2, nTDCs = 4):  # 0x1A for kc705-01 and 0x1F for kc705-02, 0x5 0x2, 0x9 0x2, 0xb 0x2, 0xc 0x2 for ac701
