...
class Communicator(object):
    ...
    def communicate_pattern(self, params):
        ...
        # tdc_data_path = '/home/anthony/Documents/fpga-phantom-circuits/src/code_4TDCs_receiver/Vivado/data/tdc_raw_data_12V/'
        tdc_data_path = '/home/anthony/Documents/fpga-phantom-circuits/src/code_4TDCs_receiver/Vivado/data/tdc_raw_data_11.99V/'
        ...
        for i in range(1):
            ...
            for j in range(repetitions):
                ...
                # save TDC_0 data to file
                dFileName = "TDC_0_data_%s"%(timeStamp())
                saveFile = "%s.npz"%(dFileName)
                print ("Saving to file %s"%(saveFile))
                np.savez_compressed(tdc_data_path + saveFile, data=np.asarray(measurements[0]))
                saveToFile(measurements[0], "%s.txt"%(tdc_data_path + dFileName))

                # save TDC_1 data to file
                dFileName = "TDC_1_data_%s"%(timeStamp())
                saveFile = "%s.npz"%(dFileName)
                print ("Saving to file %s"%(saveFile))
                np.savez_compressed(tdc_data_path + saveFile, data=np.asarray(measurements[1]))
                saveToFile(measurements[1], "%s.txt"%(tdc_data_path + dFileName))

                # save TDC_2 data to file
                dFileName = "TDC_2_data_%s"%(timeStamp())
                saveFile = "%s.npz"%(dFileName)
                print ("Saving to file %s"%(saveFile))
                np.savez_compressed(tdc_data_path + saveFile, data=np.asarray(measurements[2]))
                saveToFile(measurements[2], "%s.txt"%(tdc_data_path + dFileName))

                # save TDC_3 data to file
                dFileName = "TDC_3_data_%s"%(timeStamp())
                saveFile = "%s.npz"%(dFileName)
                print ("Saving to file %s"%(saveFile))
                np.savez_compressed(tdc_data_path + saveFile, data=np.asarray(measurements[3]))
                saveToFile(measurements[3], "%s.txt"%(tdc_data_path + dFileName))
                # sum = 0
                # for k in range(4):
    ...
    def set_up_tdc (self, ser, measures, config = 0xff, coarse = 0x1f, fine = 0x0, nTDCs = 4):  # 0x1A for kc705-01 and 0x1F for kc705-02, 0x5 0x2, 0x9 0x2, 0xb 0x2, 0xc 0x2 for ac701
