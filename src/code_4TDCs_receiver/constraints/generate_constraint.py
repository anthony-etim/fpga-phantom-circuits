...
def tdc_xdc (tdc_inst_hier, tdc_loc, clock_region, pblock_name, tdc_id = 0, tdc_xdc_lib="./ac701/tdc_carry4_X12Y150.xdc" ):
    ...
    x_offset_org = clock_region[0] 
    y_offset_org = clock_region[1] 
    xdc_constraints = []
    with open(tdc_xdc_lib, "r") as f:
        ...
        while (l != ''):
            ...
            if ('resize_pblock' in l):
                ...
                x1 = int(orig_loc[2])
                y1 = int(orig_loc[3])
                l = re.sub(r'\d+', '%d', l)%(x0+x_offset_org, y0+y_offset_org, x1+x_offset_org, y1+y_offset_org)
...
def gen_ac701(xdcfile, n = 1, places = [[12, 150]], clock_region =[[0,0]]):
    ...
    for i in range (n):
        constraints = constraints + tdc_xdc ("GEN_TDC[%d].tdc_inst" % i, places[i], clock_region[i],  "pblock_tdc_%d" % i, tdc_id=i)
    ...
    constraints = read_xdc_lib(xdcfile)
    for l in constraints:
        if ('CLOCKREGION_X158' in l):
            constraints = [word.replace('CLOCKREGION_X158', 'CLOCKREGION_X1') for word in constraints]
...
# for KC705 board
# xdcfile: final xdc file
def gen_kc705(xdcfile, n = 1, places = [[12, 150]]):
    # the constraints of tdc-0
    constraints = read_xdc_lib("./kc705/clock.xdc")
    # Add the constraints for RO heater
    constraints = constraints + read_xdc_lib("./kc705/ro_heater.xdc")

    # The constraints for 4 TDcs
    for i in range (n):
        constraints = constraints + tdc_xdc ("GEN_TDC[%d].tdc_inst" % i, places[i], "pblock_tdc_%d" % i, tdc_id=i,tdc_xdc_lib="./kc705/tdc_carry4_X12Y150.xdc")
...
if __name__ == "__main__":
    ...
    if (args.board == 'ac701'):
        # each point is the leftmost bottom point of one rectangle
        # places = [
        #     [135, 10],
        #     [20, 190],
        #     [20, 10],
        ...
        gen_ac701(xdcfile, n = args.numTDC, places = places, clock_region = clock_region)
        # gen_ac701("testGen.xdc")
    elif (args.board == 'kc705'):
        # each point is the leftmost bottom point of one rectangle
        places = [
            [12, 50],
            [88, 50],
        ...
        gen_kc705(xdcfile, n = args.numTDC, places = places)
    else:
