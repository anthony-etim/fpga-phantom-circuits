...
module carry_chain #(parameter NUM_STAGES=32) (
    ...
    assign tdc_carry[0] = tdc_wire_end;
    //// End
...
//replaced carry8 with carry4 since 7 series has no carry8
    ...
    // (*dont_touch=1*)  CARRY4 CARRY4_inst_index0(
    //     .CO       ( {carry_internal,  tdc_carry[3:1]} ),     //4-bitoutput:Carry-out
    //     .O        (carry_xor_out[3:0]),     //4-bitoutput:CarrychainXORdataout
    //     .CI       (tdc_carry[0]),           //1-bitinput:LowerCarry-In
    //     .CYINIT   (gnd),                          //1-bitinput:UpperCarry-In
    //     .DI       (4'b1111),                  //4-bitinput:Carry-MUXdatain
    //     .S        (4'b1111)                   //4-bitinput:Carry-muxselect
    // );

    // ///////////// New instantiate LUTs
    ...
    //     .O              (tdc_wire_end)
    // );
    // assign tdc_carry[4] = tdc_wire_end;
    // ////////////// End instantiate LUTs

// ERROR: [DRC DXSTAT-3] Carry input routing: Cell tdc_INST_0/u_trng_core/cc_0/CARRY4_inst_index0 has unsupported CI connectivity: invalid driver celltype. The signal tdc_INST_0/u_trng_core/cc_0/lut_out connected to cell pin CI will not route. CARRY4 carry input routing is directional for this architecture and does not use general interconnect. The COUT output from a CARRY4 cell must connect directly to the CI on a different CARRY4 cell. No intermediate logic or logical pin inversions are supported. If the connected cells are placed, the placements must be in adjacent sites with the driver cell in the lower site. Modify the cell placement or the netlist connectivity to fix this unroutable situation.
// ERROR: [DRC DXSTAT-3] Carry input routing: Cell tdc_INST_0/u_trng_core/cc_0/carry_chain_loop[1].CARRY4_inst has unsupported CI connectivity: invalid driver celltype. The signal tdc_INST_0/u_trng_core/cc_0/tdc_wire_end connected to cell pin CI will not route. CARRY4 carry input routing is directional for this architecture and does not use general interconnect. The COUT output from a CARRY4 cell must connect directly to the CI on a different CARRY4 cell. No intermediate logic or logical pin inversions are supported. If the connected cells are placed, the placements must be in adjacent sites with the driver cell in the lower site. Modify the cell placement or the netlist connectivity to fix this unroutable situation.
    ...
    dff #(.WIDTH(256)) carry_ff_0(.d(carry_xor_out),.clk(clk),.q(cdff),.reset(reset));
