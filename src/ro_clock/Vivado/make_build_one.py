...
print('Building ro_clock design for board ' + board + '...')
...
cmd = "sed -ie 's/parameter CLK_SPEED     = 300_000_000,/parameter CLK_SPEED     = %d,/g' %s"%(int(input_clk_freq_mhz)*1000000, build_dir + '/src/ro_clock.sv')
...
ipDir = '../ip/%s'%(partname)
command = '' + vivado + ' -nojournal -log ' + buildlogfile + ' -mode batch -source synthesize.tcl -tclargs ' + src + ' ' + toplevel + ' ' + partname + ' ' + xdcfile + ' ' + board + ' ' + ipDir + ' | tee -a ' + build_dir + '/synth-report.txt'
os.system(command)
