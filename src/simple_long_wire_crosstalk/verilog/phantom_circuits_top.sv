            ...
            IDLE:
                begin
                    if (rx_valid && !tx_busy) begin
                        ...
                        end else if (rx_byte == GET_THRESHOLD) begin
                            state <= SET_THRESHOLD;
                        end else if (rx_byte == GET_INTIAL_DATA) begin
                            state <= PHANTOM_DATA;
                        end else if(rx_byte == START_CROSSTALK) begin
                            //generate random 8 bits for phantom transmitter
                            state <= PHANTOM_TRANSMIT;
                        end else if (rx_byte == IS_PHANTOM_RECEIVER_READY) begin
                            state <= PHANTOM_CHECK;
                        end else if (rx_byte == GET_FINAL_DATA) begin
                            state <= FINISH;
                        end
                        ...
                        end else if (rx_byte == GET_DEBUG_ROCOUNTS_ONE) begin
                            out32counter_one <= 0;
                            state <= DEBUG_ROCOUNTS_ONE;
                        end else if (rx_byte == GET_DEBUG_ROCOUNTS_TWO) begin
                            out32counter_two <= 0;
                            state <= DEBUG_ROCOUNTS_TWO;
                        end else if (rx_byte == GET_DEBUG_ROCOUNTS_THREE) begin
                            out32counter_three <= 0;
                            state <= DEBUG_ROCOUNTS_THREE;
                        end else if (rx_byte == GET_DEBUG_ROCOUNTS_FOUR) begin
                            out32counter_four <= 0;
                            state <= DEBUG_ROCOUNTS_FOUR;
                        end else if (rx_byte == GET_DEBUG_ROCOUNTS_FIVE) begin
                            out32counter_five <= 0;
                            state <= DEBUG_ROCOUNTS_FIVE;
                        end else if (rx_byte == GET_DEBUG_ROCOUNTS_SIX) begin
                            out32counter_six <= 0;
                            state <= DEBUG_ROCOUNTS_SIX;
                        end else if (rx_byte == GET_DEBUG_ROCOUNTS_SEVEN) begin
                            out32counter_seven <= 0;
                            state <= DEBUG_ROCOUNTS_SEVEN;
                        end else if (rx_byte == GET_DEBUG_TX_WIRE) begin
                            state <= DEBUG_TX_WIRE;
                        end
                    end
                    else 
